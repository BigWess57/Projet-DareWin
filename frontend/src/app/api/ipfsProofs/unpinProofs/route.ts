...
export async function POST(request: Request) {
    try {
        ...
        const isPinned = !!(
            listJson?.rows &&
            Array.isArray(listJson.rows) &&
            listJson.rows.length > 0
        );

        if (!isPinned) {
            return NextResponse.json(
                { ok: true, message: "CID not pinned (no action)" },
                { status: 200 },
            );
        }
        ...
        const unpinRes = await fetch(
            `https://api.pinata.cloud/pinning/unpin/${encodeURIComponent(ipfsHash)}`,
            {
                method: "DELETE",
                headers: {
        ...
        if (!unpinRes.ok) {
            ...
            return NextResponse.json(
                { error: "Pinata unpin failed", details: data },
                { status: 502 },
            );
        }
        ...
        return NextResponse.json(
            {
                ...
                ipfsHash,
                pinataResponse: data,
                message: "Proofs unpinned successfully",
            },
            { status: 200 },
        );
    } catch (err) {
