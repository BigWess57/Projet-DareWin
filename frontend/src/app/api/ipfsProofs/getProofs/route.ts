...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!ipfsHash || !userAddress) {
            return NextResponse.json(
                { error: "ipfsHash and user address required" },
                { status: 400 },
            );
        }

        // Allow ipfs://CID or raw CID
        const normalizedCid = ipfsHash.startsWith("ipfs://")
        ...
        if (!res.ok) {
            return NextResponse.json(
                {
                    whitelisted: false,
                    reason: `Failed to fetch ${gatewayUrl}: ${res.status}`,
                },
                { status: 502 },
            );
        }
        ...
        if (!json?.root || !json?.proofs) {
            return NextResponse.json(
                { whitelisted: false, reason: "Invalid proof file structure" },
                { status: 422 },
            );
        }
