...
import {
    retriveEventsFromBlock,
    wagmiEventRefreshConfig,
} from "@/utils/client";
...
const VotingForWinner = ({
    ...
    const [votingDurationEnded, setVotingDurationEnded] =
        useState<boolean>(false);

    const [isRefetchingStatus, setIsRefetchingStatus] =
        useState<boolean>(false);
    ...
    //Used to check the current transaction state
    const {
        isLoading: voteEndConfirming,
        isSuccess: voteEndSuccess,
        error: voteEndReceiptError,
    } = useWaitForTransactionReceipt({
        hash: endVoteHash,
    });
    ...
    const getAllPlayers = async () => {
        try {
            ...
            //Get players from GraphQL (through next server endpoint)
            const graphQLEvents = await getPlayers(
                `/api/challenges/getAllPlayers?address=${contractAddress}`,
            );
            ...
            const rpcEvents: PlayerEvent[] = Logs.map((log) => ({
                ...
                eventType:
                    log.eventName === "PlayerJoined"
                        ? ("PlayerJoined" as const)
                        : ("PlayerWithdrawn" as const),
    ...
    useWatchContractEvent({
        address: contractAddress,
        abi: [
            parseAbiItem("event PlayerVoted(address voter, address votedFor)"),
        ],
        ...
        onLogs() {
            ...
            if (!hasEveryoneVoted) {
                readContract(config, {
                    ...
                    .catch((err) => {
                        console.error(
                            "Error retrieving remaing voters : ",
                            err,
                        );
                    });
            }
    ...
    useEffect(() => {
        ...
        const anyoneVoted =
            readData[3].result !== undefined && readData[3].result > 0
                ? true
                : false;
        ...
        if (player == undefined) {
            toast.error(
                "Error : Could not retrieve player info from contract",
                {
                    duration: 3000,
                },
            );
    ...
    const LoadingPlayersSpinner = () => (
        <div className="flex flex-col items-center justify-center p-8">
            <div className="animate-spin rounded-full h-10 w-10 border-b-4 border-blue-500 mb-4" />
            ...
            {isRefetchingStatus ? (
                ...
                <div className="space-y-8 p-6 bg-gradient-to-br from-[#1F243A] to-[#151A2A] border border-white/10 rounded-2xl shadow-xl text-white">
                            ...
                            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                                ...
                                <div className="text-center space-y-2">
                                    {hasSomeoneVoted ? (
                                        ...
                                        <div className="text-xl italic text-white/60">
                                            {t("waiting_first_vote")}
                                        </div>
                                    )}
                                </div>
                            </div>
                    ...
                    <CurrentTransactionToast
                        ...
                        isSuccess={voteSuccess}
                        successMessage={t("success_voted")}
                        txHash={voteHash}
                    />
                    <CurrentTransactionToast
                        isConfirming={voteEndConfirming}
                        isSuccess={voteEndSuccess}
                        successMessage={t("success_revealed")}
                        txHash={endVoteHash}
                    />
                </div>
            )}
