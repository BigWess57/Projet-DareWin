...
import ChallengeForm, {
    ChallengeFormValues,
} from "../Miscellaneous/ChallengeForm";
...
const ChallengeFactory = () => {
    ...
    const { data: hash, writeContract } = useWriteContract({
        mutation: {
            onError: (err) => {
                ...
                if (challengeIpfsCid) {
                    fetch("/api/ipfsProofs/unpinProofs", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ ipfsHash: challengeIpfsCid }),
                    })
                        .then(async (res) => {
                            if (!res.ok) {
                                ...
                                toast.error(
                                    t("error_unpin", {
                                        error: errJson.error ?? res.statusText,
                                    }),
                                    {
                                        duration: 3000,
                                    },
                                ...
                                console.log(
                                    response?.message ??
                                        "Unpinned (or not pinned)",
                                );
                            }
                        })
                        .catch((err) => {
                            toast.error(
                                t("network_error_unpin", {
                                    error: String(err),
                                }),
                                {
                                    duration: 3000,
    ...
    const handleCreateChallenge = async (data: ChallengeFormValues) => {
        ...
        const groupAddresses = data.groupAddresses.map(
            (elem: { address: any }) => [elem.address],
        );
        ...
        const ipfsCid = pinJson.ipfsHash; // IPFS CID (e.g. Qm...)
        ...
        if (errorConfirmation) {
            ...
            if (challengeIpfsCid) {
                fetch("/api/ipfsProofs/unpinProofs", {
                    ...
                    .catch((err) => {
                        toast.error(
                            t("network_error_unpin", { error: String(err) }),
                            {
                                duration: 3000,
                            },
                        );
