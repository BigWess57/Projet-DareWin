...
const SwapWidget = () => {
    ...
    const handlePayChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const maxBalance = activePayToken.balance
            ? parseFloat(activePayToken.balance.formatted)
            : 0;
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            try {
                ...
                console.log(
                    `Getting quote for ${payAmount} ${activePayToken.symbol}...`,
                );
        ...
        const timer = setTimeout(async () => {
            try {
                try {
                    ...
                    console.log(
                        `Getting reverse quote for ${receiveAmount} ${activeReceiveToken.symbol}...`,
                    );
    ...
    const getUsdValue = (amountStr: string, isDare: boolean = false) => {
        ...
        if (!ethPrice || isNaN(val) || val === 0 || currentDarePrice == null)
            return " _";
        if (isDare) {
    ...
    const renderPriceImpact = () => {
        if (
            typeof payUsdValue === "string" ||
            typeof receiveUsdValue === "string"
        )
            return null;
                ...
                <div className="relative bg-[#151929] border border-white/10 rounded-3xl p-4 shadow-2xl">
                        ...
                        {showSettings && (
                            ...
                            >
                                ...
                                <div className="relative">
                                    <div
                                        className={`flex items-center bg-[#0F111A] border rounded-lg px-3 py-2 ${slippage > 5 || slippage < 0.01 ? "border-yellow-500/50" : "border-white/10"}`}
                                    >
                                        <input
                                            type="number"
                                            value={slippage}
                    ...
                    >
                        <div className="flex justify-between text-sm text-gray-400 mb-2">
                            <span>{t("you_pay")}</span>
                            <span
                                className={`flex items-center gap-1 ${isInsufficientBalance ? "text-red-400" : ""}`}
                            >
                                {t("balance")}:{" "}
                                {activePayToken.loading ? (
                                    ...
                                    >
                                        {activePayToken.balance?.formatted
                                            ? Number(
                                                  activePayToken.balance
                                                      .formatted,
                                              ).toFixed(isEthToDare ? 6 : 2)
                                            : "0.00"}
                                    </span>
                                )}
                    ...
                    `}
                            ...
                            <button
                                className={`flex items-center bg-[#1F243A] text-white px-3 py-1.5 rounded-full font-medium transition-all border border-white/10 ring-2 ${isEthToDare ? "ring-blue-500/20" : "ring-purple-500/20"}`}
                            >
                                <TokenIcon
                                    symbol={activePayToken.symbol}
                                    color={activePayToken.color}
                                />
                                {activePayToken.symbol}
                            </button>
                        </div>
                        <div className="text-gray-500 text-sm mt-1 flex flex-col">
                            {isQuoteLoading ? null : (
                                <span className="text-gray-500">
                                    ...
                                    {payUsdValue.toLocaleString("en-US", {
                    ...
                    <div className="relative h-4 flex justify-center items-center z-10">
                        ...
                        >
                            <ArrowDownUp
                                size={30}
                                className={`duration-300 ${!isEthToDare ? "rotate-180" : ""}`}
                            />
                        </button>
                    </div>
                    ...
                    <div className="bg-[#0F111A] rounded-2xl p-4 mt-2 border border-white/5 hover:border-white/10 transition-colors">
                        <div className="flex justify-between text-sm text-gray-400 mb-2">
                            ...
                            <span>
                                {t("balance")}:{" "}
                                {activeReceiveToken.loading ? (
                                    <span className="animate-pulse bg-white/10 w-10 h-3 rounded"></span>
                                ) : (
                                    <span className="text-white/90">
                                        {activeReceiveToken.balance?.formatted
                                            ? Number(
                                                  activeReceiveToken.balance
                                                      .formatted,
                                              ).toFixed(isEthToDare ? 2 : 6)
                                            : "0.00"}
                                    </span>
                                )}{" "}
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <button
                                className={`flex items-center bg-[#1F243A] text-white px-3 py-1.5 rounded-full font-medium transition-all border border-white/10 ring-2 ${isEthToDare ? "ring-purple-500/20" : "ring-blue-500/20"}`}
                            >
                                <TokenIcon
                                    symbol={activeReceiveToken.symbol}
                                    color={activeReceiveToken.color}
                                />
                                {activeReceiveToken.symbol}
                            </button>
                        </div>
                    ...
                    <div className="flex justify-between items-center px-4 py-3 text-sm">
                        <div
                            className={`flex items-center font-medium cursor-pointer  ${isDarePriceError ? "text-red-500  hover:text-red-400" : "text-cyan-400  hover:text-cyan-300"}`}
                        >
                            ...
                            {isDarePriceError ? (
                                ...
                                currentDarePrice?.formattedFromEthToDare
                            ) : (
