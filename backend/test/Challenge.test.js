...
describe("tests Challenge contract", function () {
  ...
  async function deployedChallengeFixtureBase() {
    ...
    await token.transfer(
      signers[1].address,
      ethers.parseUnits("50000", await token.decimals()),
    );
    //Send a bit more to 4th signer (silver tier)
    await token.transfer(
      signers[3].address,
      ethers.parseUnits("20000", await token.decimals()),
    );
  ...
  async function OngoingChallengeFixture() {
    ...
    const { challenge, signers, token, bid } =
      await deployedChallengeFixtureBase();
    ...
    const {
      ...
      s: s1,
      deadline: deadline1,
    } = await GetRSVsig(signers[0], token, bid, challenge);
    const {
      v: v2,
      r: r2,
      s: s2,
      deadline: deadline2,
    } = await GetRSVsig(signers[1], token, bid, challenge);
    const {
      v: v3,
      r: r3,
      s: s3,
      deadline: deadline3,
    } = await GetRSVsig(signers[2], token, bid, challenge);
    const {
      v: v4,
      r: r4,
      s: s4,
      deadline: deadline4,
    } = await GetRSVsig(signers[3], token, bid, challenge);
    const {
      v: v5,
      r: r5,
      s: s5,
      deadline: deadline5,
    } = await GetRSVsig(signers[4], token, bid, challenge);
    ...
    await challenge
      .connect(signers[1])
      .joinChallenge(deadline2, v2, r2, s2, proof2);
    await challenge
      .connect(signers[2])
      .joinChallenge(deadline3, v3, r3, s3, proof3);
    await challenge
      .connect(signers[3])
      .joinChallenge(deadline4, v4, r4, s4, proof4);
    await challenge
      .connect(signers[4])
      .joinChallenge(deadline5, v5, r5, s5, proof5);
  ...
  describe("deployment", function () {
    ...
    describe("Constructor Validation Tests", function () {
      ...
      it("should revert with IPFSCidEmpty when IPFS CID is empty", async function () {
        const playersAllowed = [[signers[0].address], [signers[1].address]];
  ...
  describe("gathering players state", function () {
    ...
    beforeEach(async function () {
      ({ challenge, signers, bid, token, merkleTree } = await loadFixture(
        deployedChallengeFixtureBase,
      ));
    });

    it("should allow a player to join, and sends the money to the challenge contract correctly", async function () {
      const { v, r, s, deadline } = await GetRSVsig(
        signers[0],
        token,
        bid,
        challenge,
      ...
      expect(await token.balanceOf(challenge)).to.equal(0);

      await expect(challenge.joinChallenge(deadline, v, r, s, proof1)).to.not.be
        .reverted;

      expect(await token.balanceOf(challenge)).to.equal(bid);
    });
    ...
    it("should not allow a player to join if he is not among those chosen by the admin at creation", async function () {
      const {
        ...
        s: s1,
        deadline: deadline1,
      } = await GetRSVsig(signers[5], token, bid, challenge);
      ...
      await expect(
        challenge
          .connect(signers[5])
          .joinChallenge(deadline1, v1, r1, s1, proof1),
    ...
    it("should not allow a participant to join twice", async function () {
      ...
      await challenge.joinChallenge(deadline1, v1, r1, s1, proof1);
      await expect(
        challenge.joinChallenge(deadline1, v1, r1, s1, proof1),
      ).to.be.revertedWithCustomError(challenge, "AlreadyJoined");
    });
    ...
    it("should allow a player to withdraw from the challenge (before start), while letting the challenge start when required without issues. Money should be sent back to him", async function () {
      ...
      expect(await token.balanceOf(signers[2].address)).to.equal(bid);

      //Admin successfully starts challenge
      await challenge.startChallenge();

      expect(await challenge.currentStatus()).to.equal(
        ChallengeStatus.OngoingChallenge,
      );
    });
    ...
    it("should allow a player to join, withdraw and then join again the challenge", async function () {
      ...
      await expect(challenge.joinChallenge(deadline2, v2, r2, s2, proof2)).to
        .not.be.reverted;
    });
    ...
    it("should not allow to start the challenge if there is less than 2 players", async function () {
      ...
      await expect(challenge.startChallenge()).to.be.revertedWithCustomError(
        challenge,
        "InsufficientPlayersToStart",
      );
    });
  ...
  describe("voting for winner state", function () {
    ...
    it("should allow a player to vote (for another player).", async function () {
      await expect(challenge.voteForWinner(signers[1].address)).to.not.be
        .reverted;
    });
    ...
    it("should not allow to go to next state (ChallengeWon) if everyone has not voted (and minimum delay hasn't passed)", async function () {
      await challenge.voteForWinner(signers[1].address);
      await challenge.connect(signers[1]).voteForWinner(signers[1].address);
      await expect(challenge.endWinnerVote()).to.be.revertedWithCustomError(
        challenge,
        "VotingStillOngoing",
      );
    });
    ...
    it("should allow ANYONE to go to next state (ChallengeWon) if everyone has not voted but minimum delay has passed", async function () {
      ...
      await expect(challenge.connect(signers[2]).endWinnerVote()).not.to.be
        .reverted;
    });
  ...
  describe("challenge won state", function () {
    ...
    let token;
    beforeEach(async function () {
      ({ challenge, signers, bid, token } = await loadFixture(
        VotingForWinnerFixture,
      ));
    });
    ...
    it("When there is a tie, should allow both winners to receive prize", async function () {
      ...
      const expectedDiff2 =
        (bid * 5n) / 2n - (((bid * 5n) / 2n) * silver) / 100n;

      expect(diff2).to.equal(expectedDiff2); //substract fees too
    });
    ...
    it("fee receiver should receive half of the fees (the rest is burnt)", async function () {
      ...
      expect(diff).to.equal(expectedFeesReceived);
    });
    ...
    it("half of the fees should have been burnt", async function () {
      ...
      expect(diff).to.equal(expectedBurntAmount);
    });
  });
  ...
  describe("Owner functions", function () {
    it("should not allow not admin player to go to OngoingChallenge state", async function () {
      const { challenge, signers } = await loadFixture(
        deployedChallengeFixtureBase,
      );

      await expect(challenge.connect(signers[1]).startChallenge())
  ...
  describe("State functions availability", function () {
    describe("state GatheringPlayers", function () {
      ...
      let signers;
      beforeEach(async function () {
        ({ challenge, signers } = await loadFixture(
          deployedChallengeFixtureBase,
        ));
      });
      ...
      it("should not allow withdrawPrize() in GatheringPlayers state", async function () {
        await expect(challenge.withdrawPrize()).to.be.revertedWithCustomError(
          challenge,
          "InvalidState",
        );
      });
    ...
    describe("state OngoingChallenge", function () {
      ...
      it('should allow voteForWinner() in OngoingChallenge state (if the challenge is over). state should become "VotingForWinner" after first player votes at the end of the challenge. ', async function () {
        ...
        expect(await challenge.currentStatus()).to.equal(
          ChallengeStatus.VotingForWinner,
        );
      });
  ...
  describe("Events", function () {
    it("should emit an event when a Player Joined", async function () {
      const { challenge, signers, bid, token } = await loadFixture(
        deployedChallengeFixtureBase,
      );
      const {
        v: v1,
    ...
    it("should emit an event when ChallengeStarted", async function () {
      ...
      await expect(challenge.startChallenge()).to.emit(
        challenge,
        "ChallengeStarted",
      );
    });
    ...
    it("should emit an event when ChallengeEnded ", async function () {
      ...
      await expect(challenge.voteForWinner(signers[1].address)).to.emit(
        challenge,
        "ChallengeEnded",
      );
    });
    ...
    it("should emit an event when a prize is sent", async function () {
      ...
      await expect(await challenge.connect(signers[1]).withdrawPrize())
