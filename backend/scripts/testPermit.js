...
async function main() {
    ...
    const myToken = await ethers.deployContract("DareWin", [
        signers[0].address,
    ]);
    ...
    let tokenOwnerBalance = (
        await myToken.balanceOf(tokenOwner.address)
    ).toString();
    let tokenReceiverBalance = (
        await myToken.balanceOf(tokenReceiver.address)
    ).toString();
    ...
    tx = await myToken
        .connect(tokenReceiver)
        .transferFrom(tokenOwner.address, tokenReceiver.address, value);
